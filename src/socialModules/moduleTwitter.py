...
class moduleTwitter(Content):  # , Queue):
    ...
    def setApiPosts(self):
        ...
        posts = self.apiCall(
            self.getClient().get_home_timeline, tweet_fields=["entities"]
        )  # ,
    ...
    def processReply(self, reply):
        ...
        if isinstance(reply, str) and (
            ("You have already retweeted" in reply)
        ...
        return (
            reply  # Return the original reply if it's not a recognized success format
        )
        ...
        if len(args) == 2:
            ...
            if imageName:
                ...
                try:
                    ...
                    id_img1 = self.apiCall(
                        self.getApi().simple_upload, filename=imageName
                    )
                    ...
                    if "alt" in more:
                        ...
                        #     })
                        logging.debug(
                            f"Setting up alt: {more['alt']}" f" in image {id_img1}"
                        )
                        self.apiCall(
                            self.getApi().create_media_metadata,
