...
class Content:
    ...
    def getLastTime(self, other=None):
        ...
        logMsg(f"myLastLink: {myLastLink} {lastTime}", 2, False)
    ...
    def report(self, profile, post, link, data):
        ...
        if data:
            logMsg(f"{self.indent} Data error: {data}", 2, False)
    ...
    def getPostContent(self, post):
        res = ""
        summary = self.getPostContentHtml(post) else self.getApiPostContent(post)
