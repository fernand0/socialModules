...
class moduleImap(Content):  # , Queue):
    ...
    def selectHash(self, M, folder, hashSelect):
        ...
        for num in data[0].split():
            ...
            if msgDigest == hashSelect:
                ...
                msgLog = "Message %s %s" % (num, msgDigest)
                logMsg(msgLog, 2, False)
        ...
        msgLog = "END %d messages have been selected " % i
        logMsg(msgLog, 2, False)
        ...
        while listFolders == "":
            ...
            for name in data:
                if inNameFolder.encode("ascii").lower() in name.lower():
                    listFolders = listFolders + "%d) %s " % (i, nameFolder(name))
                    numberFolder = i
                i = i + 1
            iFolder = ""
    ...
    def publishApiPost(self, *args, **kwargs):
        ...
        if kwargs:
            ...
            logMsg(f"api: {api_src} posts: {posts} action:{api_src.more}"
    ...
    def getApiPostLink(self, msg):
        ...
        # result = f"From: {fromP} Text: {snipP}"
        result = theLink
        return result
