...
class BotElectricoRefactored:
    ...
    def publicar_grafico_consumo(
        self, destinations, title, png_path, alt_text, message
    ):
        """
    ...
    def publicar_mensaje_horario(self, destinations, message):
        ...
        for service, result in results.items():
            if result["success"]:
                ...
                logging.error(
                    f"Failed to publish message to {service}: {result['error']}"
                )
...
def ejemplo_uso_refactorizado():
    ...
    mensaje_horario = (
        "¡Buenos días! Recordatorio: usa la energía de forma responsable 🌱"
    )
