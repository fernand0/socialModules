...
def demo_optional_caching():
    ...
    class MockSocialModule:
        def __init__(self, service_name):
            ...
            self._cache_publication_if_successful = (
                Content._cache_publication_if_successful.__get__(self, MockSocialModule)
            )
            self._extract_response_link_from_reply = (
                Content._extract_response_link_from_reply.__get__(
                    self, MockSocialModule
                )
            )
    ...
    print(
        f"   Publications in cache: {after_disabled_count} (unchanged from {initial_count})"
    )
    print()
    ...
    print(
        f"   Publications in cache: {after_enabled_count} (increased by {after_enabled_count - after_disabled_count})"
    )
    print()
    ...
    config = {"auto_cache": True, "url": "https://facebook.com/mypage"}
...
def demo_configuration_options():
    ...
    print(
        f"   Environment-based: {module3.getAutoCache()} (ENABLE_PUBLICATION_CACHE={os.getenv('ENABLE_PUBLICATION_CACHE', 'false')})"
    )
    print()
