...
class moduleBuffer(Content, Queue):
    ...
    def setClient(self, url, socialNetwork=None):
        ...
        api = buffpy.api.API(
            client_id=clientId, client_secret=clientSecret, access_token=accessToken
        )
    ...
    def setPosts(self):
        ...
        for method in ["sent", "pending"]:
            if profile and ("counts" in profile) and (profile.counts[method] > 0):
...
def main():
    ...
    buf.setClient("http://fernand0-errbot.slack.com/", ("linkedin", "Fernando Tricas"))
    buf.setClient(
        "http://avecesunafoto.wordpress.com/", ("instagram", "a-veces-una-foto")
    )
    buf.setPosts()
    print(buf.getPosts())
