...
class moduleFacebook(Content):  # ,Queue):
    ...
    def setPage(self, facebookAC="me"):
        ...
        try:
            ...
            url2 = (
                f"https://graph.facebook.com/{self.client_token}/"
        ...
        if facebookAC != "me":
            for i in range(len(pages["data"])):
                msgLog = (
                    f"{self.indent} Page: {pages['data'][i]['name']} " f"{facebookAC}"
                )
                logMsg(msgLog, 2, 0)
                if pages["data"][i]["name"] == facebookAC:
                    msgLog = f"{self.indent} Selected " f"{pages['data'][i]['name']}"
                    logMsg(msgLog, 2, 0)
    ...
    def setApiPosts(self):
        ...
        posts = []
        postsF = self.getPage().get_connections(self.pageId, connection_name="posts")
        if "data" in postsF:
            for post in postsF["data"]:
                postt = self.page.get_connections(
                    post["id"], connection_name="attachments"
                )
                if "data" in postt:
    ...
    def publishApiImage(self, *args, **kwargs):
        ...
        if len(args) == 3:
            ...
            try:
                if "alt" in kwargs:
                    res = self.page.put_photo(
                        imagedata, message=post, alt_text_custom=kwargs["alt"]
                    )
                else:
...
def main():
    ...
    if testingPost:
        ...
        res = apiSrc.publishPost("Prueba texto", "https://elmundoesimperfecto.com/", "")
        print(res)
        return

    testingImages = False
    if testingImages:
        res = fc.publishImage("prueba imagen", "/tmp/prueba.png", alt="Imagen con alt")
        print(res)
        return
