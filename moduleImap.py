        ...
        for rule in RULES:
            ...
            if len(msgs) == 0:
                ...
                if status == "OK":
                    ...
                    if result[0] == "OK":
                        logging.debug(
                            "%s: %d messages have been deleted." % (msgTxt, i)
                        )
    ...
    while not msg_number.isdigit() or (startMsg < 0):
        ...
        if data[0] == "OK":
            ...
            if msg_number.isdigit():
                ...
                if msg_number[1:].isdigit():
                    printMessage(
                        M, msg_data[int(msg_number[1:])], 0, int(columns), ["Subject"]
                    )
                    return (
                        ".",
...
def printMessage(
    M, msg, rows=24, columns=80, headers=["From", "To", "Subject", "Date"]
):
        ...
        while listFolders and not iFolder.isdigit():
            ...
            if listFolders.count("\n") > 1:
                ...
                iFolder = input(
                    "Folder number ("
                    + str(numberFolder)
                    + ") [-cf] Create Folder // A string to select a smaller set of folders "
                )
                if not iFolder:
...
def listFolderNames(data, inNameFolder=""):
    ...
    for name in data:
        ...
        if (
            ...
            and name.lower().find((inNameFolder + ") ").encode("ascii").lower()) == 0
        ):
...
def selectFolder(M, moreMessages="", newFolderName="", folderM=""):
    ...
    while listFolders:
        ...
        inNameFolder = input(
            "Folder number [-cf] Create Folder // A string to select a smaller set of folders "
        )
...
def makeConnection(SERVER, USER, PASSWORD):
    ...
    while not ok:
        try:
            ...
            srvMsg = SERVER.split(".")[0]
            usrMsg = USER.split("@")[0]
            logging.info("[%s,%s] wrong password!" % (srvMsg, usrMsg))
            PASSWORD = setPassword(SERVER, USER)
...
def listMessages(M, folder):
    ...
    if data[0] == "OK":
        ...
        for i in messages:  # [-40:]: #[-numMsgs:]:
            typ, msg_data_fetch = M.fetch(
                i, "(BODY.PEEK[HEADER.FIELDS (SUBJECT FROM DATE)])"
            )
            for response_part in msg_data_fetch:
                if isinstance(response_part, tuple):
                    msg = email.message_from_bytes(response_part[1])
