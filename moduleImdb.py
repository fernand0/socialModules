...
class moduleImdb(Content, Queue):
            ...
            if i[:-5] in self.channels:
                ...
                for j in range(len(data["data"][i]["PROGRAMAS"])):
                    ...
                    if genero == self.gen:
                        ...
                        self.data.append(
                            (horaIni, horaFin, title, "-", cadName, genero)
                        )
    ...
    def getPostLine(self, post):
        ...
        line = "> [{}] - ({}) {}-{}: {}".format(
            post[3], post[4], post[0], post[1], post[2]
        )
    ...
    def extractDataMessage(self, i):
        ...
        if i < len(self.getPosts()):
            ...
            comment = "[{}] - ({}) {}-{}".format(
                average, code, self.getPostTimeIni(post), self.getPostTimeEnd(post)
            )
            theContent = ""
            if len(post) > 6:
                theContent = "\n{}\n{}\n{} ({})".format(
                    post[5 + 2], post[5 + 4], post[5 + 1], post[5 + 3]
                )
